                    ...
                    <tbody>
                        @foreach ($donations as $donation)
                            <tr class="border-b bg-white hover:bg-slate-100 dark:border-gray-700 dark:bg-gray-800">
                                ...
                                <td class="px-6 py-4">
                                    <a
                                        href="{{ $donation->project == null ? '#' : route('project.single', $donation->project_id) }}">
                                        @if ($donation->project == null)
                                            <span class="rounded-md bg-red-500 px-2 text-white">No Project
                                                Found</span>
                                        @else
                                            <span class="rounded-md bg-green-500 px-2 py-0.5 text-sm text-white">
                                ...
                                <td class="px-6 py-4">
                                    <a
                                        href="{{ $donation->contributor == null ? '#' : route('user.single', $donation->user_id) }}">
                                        @if ($donation->contributor == null)
                                            <span class="rounded-md bg-red-500 px-2 text-white">No Contributor
                                                Found</span>
                                        @else
                                ...
                                <td class="text-left">
                                    <form method="POST" action="{{ route('donation.delete', $donation->id) }}">
                                        @csrf
                                        @method('DELETE')
